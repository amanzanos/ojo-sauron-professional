...
- Comandos reconocidos: **"abre el mapa / comunidad / vigilancia"**, **"activa el modo testigo"** / **"detener grabación"**, **"envía alerta"** / **"necesito ayuda"** (misma cuenta atrás cancelable que la detección automática de sonido), **"llama a mi contacto"**, **"qué hay cerca de mí"**, **"lee los últimos reportes"**, **"cómo estoy"** (resumen de pestaña/estado de grabación), **"qué hora es"**, **"cuánta batería tengo"**, **"vibra"**, **"qué tiempo hace"**.
- **Temporizadores y recordatorios**: "pon un temporizador de 10 minutos", "recuérdame llamar a mi hermano en 20 minutos", "cancela los temporizadores". Cuando toca, avisa con voz + notificación del navegador + vibración — funciona mientras WEROS sigue abierto (aunque sea en segundo plano), no si cierras el navegador del todo.
- **Abrir apps y buscar en Google**: "abre WhatsApp / Spotify / YouTube / Gmail / Google Maps" (usa enlaces `https://`, no esquemas `app://`, para que funcionen tanto si la app está instalada como si no) y "busca en Google &lt;lo que sea&gt;". En modo manos libres puede que el navegador bloquee la ventana emergente por no venir de un toque directo — en modo normal (tocando el micrófono) funciona sin problema.
...
- **Voz masculina, tono grave y cadencia pausada** (estilo Jarvis): `speak()` elige automáticamente una voz española masculina (por nombre — Jorge, Diego, Pablo, Raúl...) con el tono bajado y el ritmo algo más lento que el de una voz de sistema normal, buscando un aire de "mayordomo". Es una heurística por nombre, no un dato de género real de la API — en Android, donde las voces suelen tener nombres en clave sin pistas, puede que no siempre acierte.
- **No es la voz del actor de doblaje de Jarvis, ni la clona.** Eso requeriría un servicio de clonación de voz por IA entrenado con grabaciones de ese actor, lo cual son dos problemas reales a la vez: la Web Speech API solo puede usar voces ya instaladas en el sistema (no hay forma de "instalar" la de un actor concreto), y clonar la voz de una persona identificable sin su permiso es un problema de derechos de imagen — además de copyright si se entrena con audio de un doblaje real. Mismo tipo de límite que el de la canción con copyright.
